...
from cqr.icp import IcpRegressor
...
def compute_one_sided_coverage(y_test,y_lower,significance,name=""):
    ...
    """
    in_the_range = np.sum((y_test >= y_lower))
    coverage = in_the_range / len(y_test) * 100
    # print("%s: Percentage in the range (expecting %.2f): %f" % (name, 100 - significance*100, coverage))
    sys.stdout.flush()
...
def compute_coverage_one_sided_2dim(y_test, y1_lower, y2_lower, significance,name=""):
    """ Compute average coverage
    ...
    y_test : numpy array, true labels (n)
    y_lower1 : numpy array, estimated lower bound for the labels (n)
    y_lower2 : numpy array, estimated lower bound for the other labels (n)
    significance : float, desired significance level
    name : string, optional output string (e.g. the method name)
    ...
    """
    in_the_range = np.sum((y_test[:, 0] >= y1_lower) 
                          & (y_test[:, 1] >= y2_lower))
    coverage = in_the_range / len(y_test) * 100
    #print("%s: Percentage in the range (expecting %.2f): %f" % (name, 100 - significance*100, coverage))
    sys.stdout.flush()
...
class QuantileForestRegressorAdapter(RegressorAdapter):
    ...
    #     return self.rfqr.feature_importances_
