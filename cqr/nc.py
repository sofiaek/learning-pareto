...
# Sofia Ek removed non CRQ-related code
...
# CQR symmetric error function
class QuantileRegLowerErrFunc(RegressionErrFunc):
    """Calculates conformalized quantile regression error.
    ...
    """
    def __init__(self):
        super(QuantileRegLowerErrFunc, self).__init__()
...
# Weighted one-sided CQR error function
class WeightedQuantileRegErrFunc(RegressionErrFunc):
    """Calculates weighted conformalized quantile regression error.
    ...
    """
    def __init__(self, cqrWeighter):
        super(WeightedQuantileRegErrFunc, self).__init__()
        self.weighter = cqrWeighter
    ...
    def apply(self, prediction, y):
        ...
        error_high = y - y_upper
        err = np.maximum(error_high,error_low)
        return error_low # TODO: add back err

    def apply_inverse(self, nc, significance):
        eta = self.weighter.get_eta(nc, significance)
        return np.vstack([eta, eta])
...
class RegressorNc(BaseModelNc):
	...
	def predict(self, x, nc, significance=None, nc_non_sorted = None):
