...
class LearnerOptimizedCrossing:
    ...
    def fit(self, x, y, epochs, batch_size, verbose=False):
        ...
        for e in range(epochs):
            ...
            if (e+1) % 100 == 0 and verbose:
                ...
                model.eval()
                preds = model(x_train)
                train_epoch_loss = self.loss_func(preds, y_train).cpu().detach().numpy()

                print("CV: Epoch {}: Train {}, Train, no dropout {}, Test {}, Best epoch {}, Best Coverage {} Best Length {} Cur Coverage {}".format(e+1, epoch_loss, train_epoch_loss, test_epoch_loss, best_epoch, best_coverage, best_avg_length, coverage))
                sys.stdout.flush()
    ...
    def predict(self, x):
        ...
        """
        self.model.eval()
        test_preds = self.model(torch.from_numpy(np.array(x, dtype=np.float32)).to(self.device).requires_grad_(False)).cpu().detach().numpy()
        if self.use_rearrangement:
            test_preds = rearrange(self.all_quantiles, self.quantile_low, self.quantile_high, test_preds)
        else:
